...
RSpec.describe "Sortition search" do
  ...
  it "displays all non-cancelled sortitions without any filters" do
    expect(subject).to include(escape_translated(sortition1.title))
    expect(subject).to include(escape_translated(sortition2.title))
    expect(subject).to include(escape_translated(sortition3.title))
    expect(subject).to include(escape_translated(sortition4.title))
    expect(subject).not_to include(escape_translated(sortition5.title))
  end
  ...
  context "when searching by text" do
    ...
    it "displays only the sortitions containing the search_text" do
      ...
      expect(subject).to include(escape_translated(sortition2.title))
      expect(subject).to include(escape_translated(sortition3.title))
      expect(subject).not_to include(escape_translated(sortition4.title))
      expect(subject).not_to include(escape_translated(sortition5.title))
    end
  end
  ...
  context "when searching by state" do
    ...
    context "and the state is cancelled" do
      ...
      it "displays only the cancelled sortition" do
        expect(subject).not_to include(escape_translated(sortition1.title))
        expect(subject).not_to include(escape_translated(sortition2.title))
        expect(subject).not_to include(escape_translated(sortition3.title))
        expect(subject).not_to include(escape_translated(sortition4.title))
        expect(subject).to include(escape_translated(sortition5.title))
      end
    end
