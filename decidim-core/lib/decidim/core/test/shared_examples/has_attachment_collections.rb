...
shared_examples_for "has attachment collections" do
  ...
  context "when are ordered by weight" do
    ...
    it "shows them ordered" do
      expect(decidim_escape_translated(first_attachment_collection.name).gsub("&quot;", "\"")).to appear_before(decidim_escape_translated(last_attachment_collection.name).gsub("&quot;", "\""))
    end
  end
