...
module Decidim
  describe PushNotificationPresenter, type: :presenter do
    ...
    context "when it is a blog post" do
      ...
      describe "#body" do
        it "returns text without links and with HTML entities unescaped" do
          expect(subject.body).to eq("The post #{sanitize_translated(resource.title)} has been published in #{sanitize_translated(resource.component.participatory_space.title)}")
        end
      end
