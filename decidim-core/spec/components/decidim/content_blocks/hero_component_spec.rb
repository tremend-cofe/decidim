...
RSpec.describe Decidim::ContentBlocks::HeroComponent, type: :component do
  ...
  context "when the content block has a background image" do
    ...
    it "uses that image's big version as background" do
      expect(render_inline(subject).to_s).to include(content_block.images_container.attached_uploader(:background_image).path(variant: :big))
    end
  end
end
