...
RSpec.describe "Participatory process search" do
  ...
  it "displays all public active processes by default" do
    expect(subject).to include(escape_translated(process1.title))
    expect(subject).to include(escape_translated(process2.title))
    expect(subject).not_to include(escape_translated(past_process.title))
    expect(subject).not_to include(escape_translated(upcoming_process.title))
  end
  ...
  context "when filtering by area" do
    ...
    it "displays matching assemblies" do
      expect(subject).to include(escape_translated(process1.title))
      expect(subject).not_to include(escape_translated(process2.title))
    end
  end
  ...
  context "when filtering by date" do
    ...
    context "and the date is set to past" do
      ...
      it "displays the past process" do
        expect(subject).not_to include(escape_translated(process1.title))
        expect(subject).not_to include(escape_translated(process2.title))
        expect(subject).to include(escape_translated(past_process.title))
        expect(subject).not_to include(escape_translated(upcoming_process.title))
      end
    end
    ...
    context "and the date is set to upcoming" do
      ...
      it "displays the upcoming process" do
        ...
        expect(subject).not_to include(escape_translated(process2.title))
        expect(subject).not_to include(escape_translated(past_process.title))
        expect(subject).to include(escape_translated(upcoming_process.title))
      end
    end
