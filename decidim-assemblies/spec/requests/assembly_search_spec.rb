...
RSpec.describe "Assembly search" do
  ...
  it "displays all assemblies by default" do
    expect(subject).to include(escape_translated(assembly1.title))
    expect(subject).to include(escape_translated(assembly2.title))
  end
  ...
  context "when filtering by assembly type" do
    ...
    it "displays matching assemblies" do
      expect(subject).to include(escape_translated(assembly1.title))
      expect(subject).not_to include(escape_translated(assembly2.title))
    end
  end
