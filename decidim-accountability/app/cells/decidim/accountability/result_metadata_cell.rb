  ...
  module Accountability
    ...
    class ResultMetadataCell < Decidim::CardMetadataCell
      ...
      def status_item_compact
        ...
        {
          text: escape_translated(status.name),
          icon: "focus-2-line"
        }
      end
      ...
      def status_item
...

        {
          ...
          icon: "focus-2-line",
          value: escape_translated(status.name)
        }
      end

      def status_description
        return unless status.present? && (description = escape_translated(status.description)).present?
