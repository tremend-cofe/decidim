...
describe Decidim::Initiatives::InitiativeSentToTechnicalValidationEvent do
  ...
  let(:email_intro) { %(The initiative "#{decidim_sanitize(resource_title)}" has been sent to technical validation. Check it out at <a href="#{admin_initiative_url}">the admin panel</a>) }
  let(:notification_title) { %(The initiative "#{decidim_sanitize(resource_title)}" has been sent to technical validation. Check it out at <a href="#{admin_initiative_path}">the admin panel</a>) }
