    ...
    module Ai
      module Admin
        class ForbiddenKeywordsController < ApplicationController
          ...
          def new
            enforce_permission_to :create, :forbidden_keywords
            @form = form(Decidim::Tools::Ai::Admin::ForbiddenKeywordForm).instance
          end
          ...
          def word
            @word ||= Decidim::Tools::Ai::ForbiddenKeyword.where(organization: current_organization).find(params[:id])
          end

          def collection
            @collection ||= paginate(Decidim::Tools::Ai::ForbiddenKeyword.where(organization: current_organization))
          end
        end
      end
    end
