...
RSpec.describe "Project search" do
  ...
  it "displays all projects within the budget without any filters" do
    expect(subject).to include(escape_translated(project1.title))
    expect(subject).to include(escape_translated(project2.title))
    expect(subject).to include(escape_translated(project3.title))
    expect(subject).not_to include(escape_translated(project4.title))
    expect(subject).not_to include(escape_translated(project5.title))
  end
  ...
  context "when searching by status" do
    ...
    context "and the status is selected" do
      ...
      it "displays the selected projects" do
        expect(subject).to include(escape_translated(project1.title))
        expect(subject).to include(escape_translated(project2.title))
        expect(subject).not_to include(escape_translated(project3.title))
      end
    end
    ...
    context "and the status is not selected" do
      ...
      it "displays the selected projects" do
        expect(subject).not_to include(escape_translated(project1.title))
        expect(subject).not_to include(escape_translated(project2.title))
        expect(subject).to include(escape_translated(project3.title))
      end
    end
  end
