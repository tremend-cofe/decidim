# frozen_string_literal: true

module Decidim
  module Budgets
    class ProjectType < Decidim::Api::Types::BaseObject
      implements Decidim::Core::ScopableInterface
      implements Decidim::Core::AttachableInterface
      implements Decidim::Comments::CommentableInterface
      implements Decidim::Core::CategorizableInterface
      implements Decidim::Core::TimestampsInterface

      description "A project"

      field :id, ID, "The internal ID for this project", null: false
      field :title, Decidim::Core::TranslatedFieldInterface, "The title for this project", null: true
      field :description, Decidim::Core::TranslatedFieldInterface, "The description for this project", null: true
      field :budget_amount, Int, "The budget amount for this project", null: true
      field :selected, Boolean, "Whether this proposal is selected or not", method: :selected?, null: true
      field :reference, String, "The reference for this project", null: true
    end
  end
end
