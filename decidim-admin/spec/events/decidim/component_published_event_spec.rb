...
describe Decidim::ComponentPublishedEvent do
  ...
  let(:email_subject) { "An update to #{decidim_sanitize(translated(participatory_space.title))}" }
  let(:resource_title) { escape_translated(resource.name) }
  let(:participatory_space_title) { escape_translated(participatory_space.title) }
